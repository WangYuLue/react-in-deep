...
但是由于原生提供的 `requestIdleCallback` 方法的 `timeRemaining()` 最大返回是 50ms，也就是 20fps，达不到页面流畅度的要求，并且该 API 兼容性也比较差。所以 React 团队没有直接使用原生的 `requestIdleCallback`，而是自己 polyfill 了一个。
...
function traversal(node) {
  if (!node) return;
  fibonacciWithTime(25);
  node.children.forEach(child => deepTraversal(child))
}
```
...
现在我们已经为这些节点建立了联系，那么如何遍历这些节点呢？React 团队核心成员 SebastianMarkbåge 在 [React issue 7942](https://github.com/facebook/react/issues/7942) 中已经为我们给出了答案：
